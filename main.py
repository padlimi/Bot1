...
# Ganti konstanta font
FONT_BOLD = None  # Akan menggunakan get_font
FONT_REGULAR = None
# ========================================
...
def draw_cell(draw, x, y, nama, harga):
    ...
    # Gunakan fungsi get_font
    font_header = get_font(bold=True, size=42)
    font_nama = get_font(bold=True, size=34)
    font_harga = get_font(bold=True, size=108)
...
def draw_cell_normal(draw, x, y, nama, harga):
    ...
    font_nama = get_font(bold=True, size=36)
    font_harga = get_font(bold=True, size=108)
    ...
    if nama:
        ...
        while current_size > 18:
            font_test = get_font(bold=True, size=current_size)
            bbox_n = draw.textbbox((0, 0), nama, font=font_test)
            if (bbox_n[2] - bbox_n[0]) <= max_w:
                font_nama = font_test
                break
            current_size -= 2
    ...
    if harga:
        ...
        while current_size > 30:
            font_test = get_font(bold=True, size=current_size)
            bbox_h = draw.textbbox((0, 0), harga_fmt, font=font_test)
            if (bbox_h[2] - bbox_h[0]) <= (CELL_W - 20):
                font_harga = font_test
                break
            current_size -= 4
...
def fit_font(text, max_w, draw, min_size=16, max_size=54):
    """Adjust font size to fit within max width"""
    current_size = max_size
    while current_size > min_size:
        font = get_font(bold=True, size=current_size)
        bbox = draw.textbbox((0, 0), text, font=font)
        if (bbox[2] - bbox[0]) <= max_w:
            return font
        current_size -= 2
    return get_font(bold=True, size=min_size)
...
def draw_cell_paket(draw, x, y, harga_normal, harga_promo):
    ...
    font_rp_norm = get_font(bold=True, size=22)
    font_rp_prom = get_font(bold=True, size=32)

    norm_fmt = format_harga(harga_normal)
    promo_fmt = format_harga(harga_promo)

    judul = "PAKET HEMAT"
    font_judul = fit_font(judul, CELL_W - 2 * PAD, draw, min_size=20, max_size=54)
    norm_font_fit = fit_font(norm_fmt, CELL_W // 2 - 40, draw, min_size=16, max_size=28)
    prom_font_fit = fit_font(promo_fmt, CELL_W - 2 * PAD - 50, draw, min_size=30, max_size=88)
