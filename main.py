...
def draw_cell_paket(draw, x, y, harga_normal, harga_promo):
    ...
    font_rp = get_font(bold=True, size=26)
    ...
    # Judul PAKET HEMAT
    judul = "PAKET HEMAT"
    font_judul = get_font(bold=True, size=50)
    ...
    font_norm = get_font(bold=True, size=30)
    bbox_norm = draw.textbbox((0, 0), norm_text, font=font_norm)
    while (bbox_norm[2] - bbox_norm[0]) > (CELL_W // 2 - 20) and font_norm.size > 18:
    ...
    font_promo = get_font(bold=True, size=60)
    bbox_promo = draw.textbbox((0, 0), promo_text, font=font_promo)
    while (bbox_promo[2] - bbox_promo[0]) > (CELL_W - 2 * PAD) and font_promo.size > 28:
